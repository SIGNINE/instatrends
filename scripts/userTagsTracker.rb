...
$tags = Hash.new
$redis.smembers("user tags").each do |tag|
    $tags[tag] = 
    Thread.new(tag) { |t|
                    begin
                        tracker(t)
...
#Start listening
redis_listener.subscribe("user") do |on|
	on.message do |channel, msg|
        ...
        if msg.has_key? "add"
            ...
            if $tags.has_key? msg["delete"]
                #For errors such as connecting to db, parsing json etc
                begin
                     Thread.kill($tags[msg["delete"]])               
                rescue Exception => e
                end
                $tags.delete msg["delete"]
            end           
        end 
	end	
end
